...
export const ja: Record<keyof Messages, string> = {
  ...
  'search.index.tiersHint': '階層は各ファイルをインデックスする際に、そのフロントマターと上の元資料パターンから決まるもので、ノートに書き戻されることはありません。「元資料」に数えられるのはパターンに一致したファイルだけなので、この数が不自然に多いときは、たいていどれかのパターンが意図より広く書かれています。フロントマターもパターン一致もないファイルは「未分類」——誰が書いたかをまだ誰も宣言していない、という意味です。ファイルを編集すればそのファイルだけが自動的に分類し直され、インデックス全体を作り直す必要はありません。',
