...
export const ja: Record<keyof Messages, string> = {
  ...
  'sotvault.noteConflict': '⚠️ サイドノートのマージ競合が発生しました — 競合マーカーが挿入され、元のファイルがバックアップされました（.conflict）。',
  ...
  'search.index.tiersHint': '階層は各ファイルをインデックスする際にフロントマターから推定されるもので、ノートに書き戻されることはありません。「元資料」の数が不自然に多い場合、多くはノートにフロントマターがないだけです。`type:` フィールド(または任意のフロントマター)を追加して保存すれば、そのファイルだけが自動的に分類し直されます。インデックス全体を作り直す必要はありません。',
  'search.index.tiersUnlabeledHint': '上の「{label}」をクリックすると、この階層の重みで押し下げられているファイルを検索パネルで絞り込んで開きます。',
