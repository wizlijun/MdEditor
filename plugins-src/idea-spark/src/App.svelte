...
<script lang="ts">
  ...
  import { agentStatus, bridge, vaultList, vaultRead } from './lib/bridge'
  ...
  /**
   * 右键「溯源」等宿主入口送来的预填文本:开一条新 idea 再整体填入。
   * 走 `keepUnsaved` 的同一道闸——预填不可以成为绕过未保存拦截的旁门。
   */
  async function applySeed(text: string): Promise<void> {
    if (!(await keepUnsaved())) return
    newIdea() // 与 startNew 同源:detach 当前文档并推进 placeholder 轮换
    showMarkdown(text)
    refreshPlaceholder()
    kit?.focus()
  }
