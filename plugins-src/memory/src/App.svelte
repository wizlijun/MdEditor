<script lang="ts">
  ...
  function closeTopLayer(event: KeyboardEvent) {
    if (event.defaultPrevented || event.isComposing || event.key !== 'Escape' || writing) return
...
<main class="ui-surface">
  ...
  {#if !showAdd && !editTarget && !destructive && !showReset && !mergeConflict && !showContext}{@render operationError()}{/if}
...
{#if showAdd && snapshot?.owner}
  <div class="scrim" role="presentation">
    <div class="sheet ui-surface" role="dialog" aria-modal="true" aria-labelledby="add-title" use:modalFocus={{ onClose: () => { showAdd = false }, canClose: () => !writing }}>
      ...
      <fieldset class="sheet-fields" disabled={writing} aria-label="主张内容与使用范围">
      ...
      {@render operationError()}<footer><button onclick={() => showAdd = false} disabled={writing}>取消</button><button class="primary" onclick={submitAdd} disabled={writing || !addText.trim() || !addSpace || !addRole || !addPurposes.length || (addNeedsAvoid && !addAvoid.trim())}>{writing ? '正在保存…' : '保存并确认'}</button></footer>
    </div>
  </div>
{/if}
...
{#if editTarget}
  <div class="scrim" role="presentation">
    <div class="sheet ui-surface" role="dialog" aria-modal="true" aria-labelledby="edit-title" use:modalFocus={{ onClose: () => { editTarget = null; editText = '' }, canClose: () => !writing }}>
      ...
      <label class="field">主张内容<textarea bind:this={editTextarea} bind:value={editText} rows="7" maxlength="32768" disabled={writing}></textarea></label>
      {@render operationError()}<footer><button onclick={() => { editTarget = null; editText = '' }} disabled={writing}>取消</button><button class="primary" onclick={submitEdit} disabled={writing || !editText.trim() || (editTarget.kind === 'confirmed' && editText.trim() === editTarget.current.claim.text)}>{writing ? '正在保存…' : editTarget.kind === 'confirmed' ? '保存修改' : '保存并确认'}</button></footer>
    </div>
  </div>
{/if}

{#if destructive}
  <div class="scrim" role="presentation"><div class="sheet compact-sheet ui-surface" role="alertdialog" aria-modal="true" aria-labelledby="destructive-title" use:modalFocus={{ onClose: () => { destructive = null }, canClose: () => !writing }}><header><div><h2 id="destructive-title">{destructive.kind === 'batch-pending' ? `批量${batchActionText(destructive.action)} ${destructive.pending.length} 条建议？` : destructive.kind === 'reject' ? '否认这条建议？' : destructive.kind === 'delete-candidate' ? '删除这个候选？' : destructive.kind === 'approve-lifecycle' ? `确认${destructive.pending.revision.lifecycle.state === 'deleted' ? '删除' : '撤销'}此主张？` : '移出当前记忆？'}</h2><p>{destructive.kind === 'batch-pending' ? destructive.action === 'ignore' ? `这 ${destructive.pending.length} 条建议会离开待确认队列，并分别记录忽略决定，避免相同内容反复出现。` : destructive.action === 'reject' ? `系统会分别记录对这 ${destructive.pending.length} 条建议的否认决定；它们不会进入当前记忆。` : `这 ${destructive.pending.length} 个候选会从待确认队列移除；已有 Git 历史可能继续保留。` : destructive.kind === 'delete-claim' ? '主张会从当前投影和 Agent context 移除；不可变历史仍会保留。这不是从 Git 历史永久擦除。' : destructive.kind === 'approve-lifecycle' ? '这是不可见性变更：主张会离开当前投影和 Agent context；不可变历史仍会保留。' : destructive.kind === 'reject' ? '系统会记录否认决定，建议不会进入当前记忆。' : '候选不会进入当前记忆；已有 Git 历史可能继续保留。'}</p></div></header>{@render operationError()}<footer><button onclick={() => destructive = null} disabled={writing}>取消</button><button class={destructive.kind === 'batch-pending' && destructive.action === 'ignore' ? 'primary' : 'destructive'} onclick={confirmDestructive} disabled={writing}>{writing ? '正在处理…' : destructive.kind === 'batch-pending' ? `确认${batchActionText(destructive.action)} ${destructive.pending.length} 条` : '确认'}</button></footer></div></div>
{/if}

{#if showReset && snapshot}
  <div class="scrim" role="presentation">
    <div class="sheet compact-sheet reset-sheet ui-surface" role="alertdialog" aria-modal="true" aria-labelledby="reset-title" use:modalFocus={{ onClose: () => { showReset = false }, canClose: () => !writing }}>
      ...
      {@render operationError()}<footer><button onclick={() => showReset = false} disabled={writing}>取消</button><button class="destructive" onclick={resetAllMemory} disabled={writing}>{writing ? '正在清理…' : '确认清空并重构'}</button></footer>
    </div>
  </div>
{/if}

{#if mergeConflict}
  <div class="scrim" role="presentation"><div class="sheet ui-surface" role="dialog" aria-modal="true" aria-labelledby="merge-title" use:modalFocus={{ onClose: () => { mergeConflict = null }, canClose: () => !writing }}><header><div><h2 id="merge-title">合并并发版本</h2><p>合并会创建同时引用所有当前 head 的新 revision。</p></div><button class="close" aria-label="关闭合并编辑" onclick={() => mergeConflict = null} disabled={writing}>×</button></header><label class="field">合并后的主张<textarea bind:value={mergedText} rows="7" disabled={writing}></textarea></label>{@render operationError()}<footer><button onclick={() => mergeConflict = null} disabled={writing}>取消</button><button class="primary" onclick={() => resolveConflict(mergeConflict!, 'merge')} disabled={writing || !mergedText.trim()}>{writing ? '正在保存…' : '保存合并'}</button></footer></div></div>
{/if}

{#if showContext && snapshot}
  <div class="scrim" role="presentation"><div class="sheet context-sheet ui-surface" role="dialog" aria-modal="true" aria-labelledby="context-title" use:modalFocus={{ onClose: () => { showContext = false }, canClose: () => !writing }}><header><div><h2 id="context-title">Context Manifest</h2><p>先明确 Role、Scope、用途和接收方，再查看将提供给 Agent 的最小主张集合。</p></div><button class="close" aria-label="关闭 Context Manifest" onclick={() => showContext = false}>×</button></header>
    ...
    {@render operationError()}<div class="context-actions"><button class="primary" onclick={previewContext} disabled={contextBusy || !contextSpace || !contextPurpose || !contextProvider || !contextModel || !contextAsOf}>{contextBusy ? '正在检查…' : '预览选择'}</button></div>
...
<style>
  .sheet-fields { display: block; min-width: 0; padding: 0; margin: 0; border: 0; }
  :global(#memory-app){height:100vh;overflow:auto;overscroll-behavior:contain}
  :global(:root){color-scheme:light dark;font:13px/1.45 -apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif;accent-color:var(--ui-accent-text)}:global(body){margin:0;background:Canvas;color:CanvasText}:global(*){box-sizing:border-box}:global(button),:global(input),:global(select),:global(textarea),:global(summary){font:inherit;pointer-events:auto;-webkit-app-region:no-drag}:global(button){min-height:32px;border:1px solid color-mix(in srgb,CanvasText 14%,transparent);border-radius:8px;padding:0 12px;background:color-mix(in srgb,CanvasText 5%,Canvas);color:CanvasText;cursor:pointer}:global(button:hover:not(:disabled)){background:color-mix(in srgb,CanvasText 10%,Canvas)}:global(button:active:not(:disabled)){transform:translateY(1px)}:global(button:focus-visible),:global(input:focus-visible),:global(select:focus-visible),:global(textarea:focus-visible),:global(summary:focus-visible){outline:3px solid color-mix(in srgb,#0a84ff 34%,transparent);outline-offset:2px}:global(button:disabled){opacity:.42;cursor:default}:global(button.primary){min-height:34px;background:var(--ui-accent);border-color:var(--ui-accent-text);color:white;font-weight:600}:global(button.primary:hover:not(:disabled)){background:#0077ed}:global(button.destructive){background:#ff3b30;border-color:var(--ui-danger);color:#fff;font-weight:600}:global(input),:global(select),:global(textarea){width:100%;min-height:32px;padding:6px 9px;border:1px solid color-mix(in srgb,CanvasText 16%,transparent);border-radius:7px;background:Canvas;color:CanvasText}:global(textarea){resize:vertical;line-height:1.5}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
  main{max-width:1240px;margin:0 auto;padding:20px 24px 40px}.app-header{display:flex;justify-content:space-between;align-items:flex-start;gap:20px}.app-header h1{margin:0;font-size:20px;line-height:1.25;letter-spacing:-.02em}.app-header p{max-width:680px;margin:4px 0 0;font-size:12px;color: var(--ui-secondary)}.header-actions{display:flex;align-items:center;gap:8px}.inference-action{display:flex;align-items:center;gap:4px}.health{display:flex;align-items:center;gap:7px;max-width:250px;color: var(--ui-secondary);font-size:12px}.health>span:first-child{flex:0 0 8px;width:8px;height:8px;border-radius:50%;background:#34c759}.health.conflict>span:first-child{background:#ff9f0a}.health.bad>span:first-child{background:#ff3b30}.icon{width:32px;padding:0;font-size:17px}.secondary{background:transparent}.reset-trigger{color:var(--ui-danger)}.banner{display:flex;align-items:flex-start;gap:10px;margin-top:14px;padding:10px 12px;border:1px solid;border-radius:9px;font-size:12px}.banner strong{white-space:nowrap}.banner.error{border-color:color-mix(in srgb,#ff3b30 42%,transparent);background:color-mix(in srgb,#ff3b30 9%,Canvas)}.banner.warning{border-color:color-mix(in srgb,#ff9f0a 45%,transparent);background:color-mix(in srgb,#ff9f0a 9%,Canvas)}.banner.inference-progress{border-color:color-mix(in srgb,#0a84ff 36%,transparent);background:color-mix(in srgb,#0a84ff 7%,Canvas)}
  .segments{display:grid;grid-template-columns:repeat(5,1fr);gap:3px;max-width:900px;margin:18px auto 16px;padding:3px;border-radius:9px;background:color-mix(in srgb,CanvasText 8%,Canvas)}.segments button{min-height:34px;border:0;background:transparent;color: var(--ui-secondary);font-weight:600}.segments button.active{background:Canvas;color:CanvasText;box-shadow:0 1px 4px rgba(0,0,0,.16)}.count{display:inline-grid;place-items:center;min-width:18px;height:18px;padding:0 5px;border-radius:9px;background:color-mix(in srgb,CanvasText 10%,Canvas);font-size: 12px}.experimental{margin-left:3px;color:var(--ui-accent-text);font-size: 12px;font-weight:750}.context-count{margin-left:3px;color:color-mix(in srgb,currentColor 68%,transparent);font-size: 12px;font-variant-numeric:tabular-nums}.count.large{min-width:28px;height:24px;border-radius:12px}.toolbar{display:grid;grid-template-columns:minmax(180px,1fr) 160px 140px auto auto;gap:8px;margin-bottom:12px}.split{display:grid;grid-template-columns:350px minmax(0,1fr);min-height:510px;border:1px solid color-mix(in srgb,CanvasText 13%,transparent);border-radius:11px;overflow:hidden;background:color-mix(in srgb,CanvasText 1.5%,Canvas)}
  .master{max-height:calc(100vh - 220px);min-height:510px;overflow:auto;border-right:1px solid color-mix(in srgb,CanvasText 12%,transparent);background:color-mix(in srgb,CanvasText 3%,Canvas)}.claim-section>h3{position:sticky;top:0;z-index:2;margin:0;padding:8px 12px 6px;border-bottom:1px solid color-mix(in srgb,CanvasText 10%,transparent);background:color-mix(in srgb,CanvasText 9%,Canvas);font-size: 12px;letter-spacing:.04em}.claim-category>h4{display:flex;justify-content:space-between;margin:0;padding:6px 12px;border-bottom:1px solid color-mix(in srgb,CanvasText 7%,transparent);background:color-mix(in srgb,CanvasText 5%,Canvas);color: var(--ui-secondary);font-size: 12px;font-weight:600}.claim-category>h4 span{font-variant-numeric:tabular-nums}.master [role=option]{display:grid;grid-template-columns:8px minmax(0,1fr) auto;align-items:start;gap:9px;width:100%;min-height:70px;padding:11px 12px;border:0;border-bottom:1px solid color-mix(in srgb,CanvasText 8%,transparent);border-radius:0;background:transparent;text-align:left}.master [role=option].selected{background:var(--ui-accent);color:#fff}.master [role=option].selected small,.master [role=option].selected .row-state{color:rgba(255,255,255,.8)}.master [role=option].sensitive{box-shadow:inset 3px 0 #ff9f0a}.master strong{display:-webkit-box;overflow:hidden;line-clamp:2;-webkit-line-clamp:2;-webkit-box-orient:vertical;font-size:13px;line-height:1.4;white-space:pre-wrap}.master small{display:block;margin-top:4px;color: var(--ui-secondary);font-size:12px}.row-state{color: var(--ui-secondary);font-size: 12px}.polarity{width:7px;height:7px;margin-top:5px;border-radius:50%;background:#8e8e93}.polarity.positive{background:#34c759}.polarity.negative{background:#ff3b30}
  .detail{min-width:0;padding:22px 24px}.detail h2{margin:12px 0 16px;font-size:17px;line-height:1.5;letter-spacing:-.01em;white-space:pre-wrap}.eyebrow{display:flex;flex-wrap:wrap;gap:6px}.badge{padding:3px 7px;border-radius:6px;background:color-mix(in srgb,CanvasText 7%,Canvas);font-size: 12px;font-weight:650}.badge.positive{background:color-mix(in srgb,#34c759 18%,Canvas);color:var(--ui-success)}.badge.negative,.badge.danger{background:color-mix(in srgb,#ff3b30 16%,Canvas);color:var(--ui-danger)}.badge.pinned{background:color-mix(in srgb,#ff9f0a 20%,Canvas);color:var(--ui-warning)}.guidance{margin-bottom:16px;padding:13px 14px;border-left:3px solid #0a84ff;border-radius:0 8px 8px 0;background:color-mix(in srgb,#0a84ff 7%,Canvas)}.guidance small,.approval-meaning small{font-size: 12px;font-weight:700;color:var(--ui-accent-text)}.guidance p{margin:4px 0 9px;font-size:13px}.guidance p:last-child{margin-bottom:0}.guidance .avoid{color:var(--ui-danger);font-weight:600}.facts{display:grid;grid-template-columns:1fr 1fr;gap:1px;margin:0;background:color-mix(in srgb,CanvasText 8%,transparent);border:1px solid color-mix(in srgb,CanvasText 8%,transparent);border-radius:8px;overflow:hidden}.facts>div{display:flex;justify-content:space-between;gap:12px;padding:9px 10px;background:Canvas}.facts>div.wide{grid-column:1/-1}.facts dt{font-size:12px;color: var(--ui-secondary)}.facts dd{margin:0;text-align:right;font-size:12px;overflow-wrap:anywhere}.detail-actions{display:flex;justify-content:flex-end;margin-top:16px}.menu-anchor{position:relative}.menu-panel{position:absolute;right:0;bottom:40px;z-index:20;min-width:190px}.menu-panel.pending-context-menu{position:fixed;right:auto;bottom:auto;z-index:90;min-width:230px}.menu-heading{padding:6px 10px 5px;color: var(--ui-secondary);font-size: 12px;font-weight:650}.menu-panel .menu-row{width:100%;min-height:30px;border:0;text-align:left}.menu-panel .danger-row{color:var(--ui-danger)}.menu-panel .danger-row:hover{background:#ff3b30;color:#fff}.approval-meaning{margin:0 0 14px;padding:12px 13px;border-radius:9px;background:color-mix(in srgb,#0a84ff 7%,Canvas)}.approval-meaning strong{display:block;margin-top:3px;font-size:14px}.approval-meaning p{margin:4px 0 0;color: var(--ui-secondary);font-size:12px}.diff{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin:12px 0 16px}.diff>div{min-height:92px;padding:11px;border-radius:8px;background:color-mix(in srgb,CanvasText 5%,Canvas)}.diff small{font-size: 12px;color: var(--ui-secondary)}.diff p{margin:5px 0 0;white-space:pre-wrap}.pending-actions{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:8px;margin-top:16px}.bulk-pending-summary>p{color: var(--ui-secondary)}.pending-selection-preview{max-height:210px;margin:14px 0 0;padding:0;overflow:auto;list-style:none;border:1px solid color-mix(in srgb,CanvasText 10%,transparent);border-radius:9px}.pending-selection-preview li{padding:8px 10px;border-bottom:1px solid color-mix(in srgb,CanvasText 8%,transparent);white-space:pre-wrap}.pending-selection-preview li:last-child{border-bottom:0}
  .history{display:grid;gap:12px}.history>header{display:flex;justify-content:space-between;align-items:center}.history h2{margin:0;font-size:16px}.history header p{margin:3px 0 0;color: var(--ui-secondary);font-size:12px}.conflict-card{padding:16px;border:1px solid color-mix(in srgb,#ff9f0a 42%,transparent);border-radius:11px;background:color-mix(in srgb,#ff9f0a 5%,Canvas)}.conflict-title{display:flex;justify-content:space-between;gap:18px}.conflict-title h3{margin:7px 0 0;font-size:15px}.conflict-title>strong{color:var(--ui-warning);font-size:12px}.ancestor{margin:12px 0;padding:10px 11px;border-radius:8px;background:color-mix(in srgb,CanvasText 5%,Canvas)}.ancestor small,.heads small{font-size: 12px;color: var(--ui-secondary)}.ancestor p,.heads p{margin:4px 0 0;white-space:pre-wrap}.heads{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.heads section{padding:11px;border:1px solid color-mix(in srgb,CanvasText 11%,transparent);border-radius:9px;background:Canvas}.heads button{margin-top:10px}.conflict-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:12px}.history-heading{margin-top:12px}.timeline{margin:0;padding:0;list-style:none;border:1px solid color-mix(in srgb,CanvasText 10%,transparent);border-radius:10px;overflow:hidden}.timeline li{display:grid;grid-template-columns:10px minmax(0,1fr) auto;gap:10px;align-items:start;padding:11px 13px;border-bottom:1px solid color-mix(in srgb,CanvasText 8%,transparent)}.timeline li:last-child{border-bottom:0}.timeline-dot{width:8px;height:8px;margin-top:5px;border-radius:50%;background:var(--ui-accent)}.timeline strong{display:block;font-size:13px}.timeline small{display:block;margin-top:2px;color: var(--ui-secondary);font-size:12px}.timeline>li>span:last-child{font-size: 12px}.empty{padding:52px 18px;text-align:center;color: var(--ui-secondary)}.empty.compact{padding:22px}.detail-empty{display:grid;place-items:center}
  .scrim{position:fixed;inset:0;z-index:100;display:grid;place-items:center;padding:24px;background:rgba(0,0,0,.28);backdrop-filter:blur(8px)}.sheet{box-sizing:border-box;width:min(680px,100%);max-height:calc(100vh - 48px);overflow:auto;padding:22px;border:1px solid color-mix(in srgb,CanvasText 15%,transparent);border-radius:14px;background:Canvas;color:CanvasText;box-shadow:0 24px 70px rgba(0,0,0,.34)}.compact-sheet{width:min(470px,100%)}.context-sheet{width:min(760px,100%)}.sheet>header{display:flex;justify-content:space-between;gap:18px}.sheet h2{margin:0;font-size:17px}.sheet header p{margin:4px 0 0;color: var(--ui-secondary);font-size:12px}.reset-sheet{border-color:color-mix(in srgb,#ff3b30 35%,transparent)}.reset-impact{margin-top:16px;padding:12px 13px;border-radius:9px;background:color-mix(in srgb,#ff3b30 8%,Canvas)}.reset-impact>strong{color:var(--ui-danger)}.reset-impact ul{margin:9px 0 0;padding-left:20px}.reset-impact li{margin:5px 0}.close{width:32px;padding:0;border:0;background:transparent;font-size:20px}.field,.form-grid label{display:grid;gap:4px;margin-top:12px;color: var(--ui-secondary);font-size:12px}.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0 10px}.form-grid .wide{grid-column:1/-1}details{margin-top:14px;padding:10px 12px;border:1px solid color-mix(in srgb,CanvasText 10%,transparent);border-radius:9px}summary{cursor:pointer;font-weight:600}fieldset{display:flex;flex-wrap:wrap;gap:10px;margin:14px 0 0;padding:10px;border:1px solid color-mix(in srgb,CanvasText 10%,transparent);border-radius:8px}legend{padding:0 4px;font-size:12px}.check{display:flex!important;align-items:center;gap:6px!important;margin:0!important}.check input{width:auto;min-height:auto}.sheet footer{display:flex;justify-content:flex-end;gap:8px;margin-top:18px}.context-actions{display:flex;justify-content:flex-end;margin-top:14px}.context-result{margin-top:14px;padding:13px;border-radius:10px;background:color-mix(in srgb,CanvasText 4%,Canvas)}.result-summary{display:flex;justify-content:space-between}.context-result ul{margin:10px 0;padding-left:20px}.context-result li{margin:5px 0}.context-result li small,.readonly-note{display:block;color: var(--ui-secondary);font-size:12px}
