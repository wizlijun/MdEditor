<script lang="ts">
  ...
  import { describeDelta, filterEntries, pendingProposalForEntry, pendingProposals, usageRule } from './lib/domain'
  import type { DecisionIntent } from './lib/domain'
  ...
  let confirmation = $state<{ proposal: Proposal; action: 'approve' | 'reject'; actor: string; intent: DecisionIntent } | null>(null), confirmMigration = $state(false)
  ...
  async function quickPriority(entry: MemoryEntry, priority: 'high'|'low', intent: 'important'|'ignore') {
    requireRegularEntry(entry)
    if (entry.status !== 'active') { error = '只能调整已确认事实的重要程度。'; return }
    await createProposal({ scope:entry.scope, operation:'set-priority', text:'', source:humanSource(), target:entry, priority, intent, dedupeKey:quickKey(intent,entry), reason:intent === 'important' ? 'Vault owner marks this fact as important.' : 'Vault owner marks this fact as safe to ignore in normal context.' })
  }
  async function quickDelete(entry: MemoryEntry) {
    requireRegularEntry(entry)
    if (entry.status === 'pending') { error = '请先确认或否认待审事实，再删除已收录的记忆。'; return }
    await createProposal({ scope:entry.scope, operation:'delete', text:'', source:humanSource(), target:entry, intent:'delete', dedupeKey:quickKey('delete',entry), reason:'Vault owner requests removal from the current projection; retain candidate and decision audit.' })
  }
  function requestDecision(proposal: Proposal, action: 'approve'|'reject', intent: DecisionIntent = 'review') {
...
<main>
  ...
  {#if tab==='current' && snapshot}
    ...
    <section class="split">
      <aside class="master" aria-label="Memory entries">
        {#each visibleEntries as entry (entry.id)}
          <button class:selected={selectedEntry?.id===entry.id} onclick={()=>selectedEntryId=entry.id}>
            <span class="polarity {entry.polarity}" aria-label={entry.polarity}></span>
            <span><strong>{entry.text}</strong><small>{entry.status} · {entry.epistemic_status} · {entry.certainty}</small></span>
            <em class:critical={entry.priority==='critical'}>{entry.priority}</em>
          </button>
        {:else}<div class="empty">{t('noEntries')}</div>{/each}
      </aside>
      {#if selectedEntry}
        <article class="detail">
          <div class="eyebrow"><span class="badge {selectedEntry.polarity}">{selectedEntry.polarity==='positive'?'正向 · 遵循':selectedEntry.polarity==='negative'?'负向 · 避免':'中性 · 背景'}</span><span class="badge priority-{selectedEntry.priority}">{selectedEntry.priority}</span><span class="badge">{selectedEntry.status}</span></div>
          ...
          <section class="quick-review" aria-label="事实快捷审阅">
            ...
            <div class="quick-actions">
              ...
              {#if selectedEntry.status==='active'}
                <button onclick={()=>quickPriority(selectedEntry,'high','important')} disabled={writing||snapshot.integrity.drift||selectedEntry.priority==='high'}>重要</button>
                <button onclick={()=>quickPriority(selectedEntry,'low','ignore')} disabled={writing||snapshot.integrity.drift||selectedEntry.priority==='low'}>可忽略</button>
              {/if}
              {#if selectedEntry.status!=='pending'}<button class="delete-action" onclick={()=>quickDelete(selectedEntry)} disabled={writing||snapshot.integrity.drift}>删除…</button>{/if}
            </div>
          </section>
