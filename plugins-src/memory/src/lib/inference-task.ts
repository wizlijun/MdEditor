...
const INSTRUCTIONS = [
  ...
  '绝不调用 approve、reject、ignore、delete、resolve、reset 或 rebuild；绝不直接编辑 `.notemd/memory/**/*.yaml` 或 `MEMORY.md`，也不修改来源文档。',
  '状态文件 `${VAULT}/.notemd/memory/.local/inference-state.json` 是唯一允许直接写入的运行缓存。',
  '',
  '开始时执行 `notemd memory owner --json`、`notemd memory snapshot --json` 与 `notemd memory check --json`。',
  ...
  '- 排除 `.git/**`、`.notemd/**`、`MEMORY.md`、依赖/构建目录、二进制、附件、`.env*`、凭据，以及 frontmatter 明确标为 `generated.by: process:*` 的生成物。',
