<script lang="ts">
  ...
  function groupName(id: string): string {
    const known: Record<string, MessageKey> = {
      'notemd.core': 'tasks.group.core',
      'notemd.ebook-import': 'tasks.group.ebook',
      'notemd.idea-spark': 'tasks.group.idea',
      'notemd.memory': 'tasks.group.memory',
      'notemd.trace-source': 'tasks.group.trace',
      'agent-tools': 'tasks.group.agent',
      custom: 'tasks.group.custom',
    }
...
{#each groups as group, index (group.id)}
  ...
  <div class="task-group">
    <button class="group-toggle" aria-expanded={expanded.has(group.id)} aria-controls={panelId} onclick={() => toggle(group.id)}>
      ...
      <span class="group-name">{groupName(group.id)}</span>
      {#if group.running}<span class="dot" title={label('status.running')}></span>{/if}
...
<style>
  ...
  .group-toggle {
    ...
    border: 1px solid var(--window-border); border-radius: 9px; background: var(--card-surface);
