...
> 类型：产品与交互设计 · 日期：2026-09-03 · 状态：待评审，不进入实现
...
4. **回答**：只有用户点击「问 Agent」后，才调用所选无头 Agent；搜索本身不消耗模型额度。
...
**快速输入态**：约 680×170，浮在当前应用上方，只含多行输入、范围、Agent 选择和最近查询。适合一句话或语音转写。

**结果工作台**：提交查询后扩展到约 940×680。左侧保留搜索结果，右侧显示选中结果预览或 Agent 回答。窗口隐藏后任务继续；再次唤起恢复本次会话。
...
- `Enter` 搜索；`Shift + Enter` 换行；`Cmd/Ctrl + Enter` 搜索并在结果到达后问当前 Agent。
- 输入法 composition 期间不触发搜索。系统听写结束后不自动提交，让用户先修改转写文本。
- 短查询停顿后可做浅搜索；多行或明显句子型长输入等待显式 `Enter`，避免对一段尚未说完的话反复深搜。
...
- 浅搜索：沿用当前索引快速返回。
- 深搜索：浅搜索为空或用户按 Enter 时，在 4 秒预算内做现有 deep scan；部分结果必须标为「搜索提前结束」。
...
- 单击结果：右侧预览。
- `Enter` 或双击：调用新增的跨窗口 `editor_show_and_reveal_search_hit({path,line,anchor})`。Rust 先显示/聚焦 `main`，主窗口再 `await openFile()` 并按最终打开路径执行 `requestReveal()`。
...
按钮形态为「问 Agent  [Claude ▾]」，运行和选择是两个控件，避免误触花费模型额度。
...
点击「问 Agent」后，由可信宿主新增的只读 `memory_context_preview` 执行：
...
| 状态 | 主信息 | 可用动作 |
| --- | --- | --- |
| 未查询 | 最近 5 次查询 / 系统听写提示 | 输入、搜索 |
| 搜索中 | 「正在搜索本地 Vault…」；旧结果可保留并变淡 | 修改查询、取消 |
...
- 从 `SearchPanel.svelte` 抽出可复用的 query controller；侧栏旧搜索保持不变。
...
2. 粘贴 300–1000 字口语转写，UI 不对半成品重复深搜，提交后能显示提取词和逐步放宽结果。
3. 每条结果能解释来源并跳到正确文件和行；回答引用走同一跳转。
4. 结果分组只依据现有 `origin` / `conceptType`，不把未标注内容冒充原始资料或人工笔记。
5. 搜索无需 Agent；问答只在明确点击后启动，且固定到本次选定 provider。
...
13. 320px、736px 和 1024px 宽度下无重叠/裁切；键盘、IME、深浅色和 popup menu hover 实机通过。
