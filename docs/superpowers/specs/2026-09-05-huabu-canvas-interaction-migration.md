...
1. 纯 TypeScript 几何与领域函数：吸附、对齐、分布、套索命中、多选缩放、分组闭包、自动端点与障碍感知评分。
...
- 单节点自由缩放；多选整体缩放，并支持 Shift 等比缩放和方向键操作缩放句柄。
- group 几何闭包复制、拖动与“适配内容”；新建/粘贴优先落到最后指针位置。
- 触控套索与框组；第二触点取消自定义绘制并交还 Flow 进行双指导航。
- 连线自环拒绝、智能相向端点、障碍感知的端点方向选择、主体补连、空白处创建标准节点并连接；边本身仍使用 Bezier 渲染，不宣称路径级绕障。
...
4. 补齐框组、自动平移、连线、标签、LOD、容器适配和障碍感知端点选择。
5. 评审收口触控、合法端点、性能、响应式默认值、键盘可访问性和菜单一致性。
6. 运行定向测试、全量测试、Svelte 检查、生产构建和差异审计。

## 7. 验证结果

- 原迁移收口定向测试：62/62 通过；发版审计后的 Canvas 测试：16/16 文件、114/114 测试通过。
- 最终 RC 全量 Vitest：253/253 文件、2,707/2,707 测试通过。
- `svelte-check`：0 error；43 个 warning 均为仓库既有告警。
- Vite production build 与 Editor Kit build check：通过。
- Canvas Rust 定向测试 25/25、移动配置测试 3/3、`aarch64-apple-ios` 与 `x86_64-apple-darwin` Rust target check：通过。
- 生产依赖 high 级审计：0；仍有 5 个既有 moderate。
- `git diff --check`：通过。
- 本机趋势穿刺：10k/20k 节点等距吸附约 5.5/6.3ms，约 10k 节点的障碍感知端点评分约 35ms，10k 节点索引套索约 2.8ms。

自动化已经覆盖 pointer 触控路径和第二触点接管。若本轮包含 iOS 包，发版前仍须在 iPad/iPhone 各做一次真实双指缩放、系统 pointer-cancel 和边缘自动平移手感穿刺。仓库当前没有 Android 工程，本轮不能宣称 Android WebView 已验收；默认 macOS 发布不包含该平台。

本轮发版审计另外修复了非活动卡片主体不可拖动、外部重载后旧手势可能回写、标签关闭前未 flush、工具栏节点落点错误、跨工作区粘贴坏引用、多文件 drop 多事务、资源缓存不可重试、Save As 后编辑器继续使用旧资源根、输入控件快捷键越权、触控命中区不足 44px，以及大选区有效 ID 检查的平方复杂度。发布脚本现在也会拦截未跟踪文件，并在版本变更前执行类型、high 级依赖安全和 Canvas 原生测试。
...
- 常驻操作改为底部居中的图标工具坞，包含选择、平移、套索、锁定、新建节点、框组、粘贴和撤销/重做；保留对应快捷键、标题和可访问名称。
...
移植到当前发布基线后还补齐了弹层键盘隔离、工具栏指针落点隔离、多连线删除入口、保存前标签输入 flush、全局菜单 hover 契约，以及 viewport/侧栏尺寸变化时的浮条边界更新。发布候选验证结果：Canvas 定向测试 16/16 文件、119/119 项；全量 Vitest 253/253 文件、2,713/2,713 项；`svelte-check` 0 error（43 个仓库既有 warning）；生产构建及 Editor Kit v1/v2 build check 通过。
