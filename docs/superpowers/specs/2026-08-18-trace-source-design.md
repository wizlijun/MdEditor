...
> **2026-08-18 演进**:溯源已从 idea-spark 中独立为专属插件
> `notemd.trace-source`(plugins-src/trace-source),右键「溯源」仅在该插件
> 已安装时显示,委托窗打开即溯源模式——`/指令` 抽象与 idea-spark 的指令
> 发现/解析(§「核心抽象」与组件 2、3)随之整体移除;`TaskDef.directive`
> 字段保留但无消费者。协议键改为语言中立的 `Source-Doc:` / `Output:`。
> 本文其余部分(模板协议、产物形状、安全、降级)仍然有效。
