...
### 4.5 项目标记：归属线索，不是第二套项目状态

每次安放都可附一个可选 `project` 标记，用于在 WIP、Waiting、Dormant 与 Closed 间保留归属和搜索线索。表单既允许自由填写，也列出当前 Next 中已使用的项目标记；历史 `transferred/project` 的目标也可成为候选。卡片显示当前项目标记，但项目不会占用额外 WIP，也不会改变安放状态。

事件字段语义为：省略 `project` 表示继承当前标记，非空字符串表示设置或更换，`null` 表示显式清除。选择“升级为项目”时复用同一个项目输入，并同时写入 `project` 与旧版已认识的 `target`，避免重复填写且保持降级兼容。

项目标记只存在于 Next 事件，不扫描不存在统一契约的 Project 文件，不自动创建项目、不改写原始 idea，也不做项目双向状态同步；真正的执行和验收仍由项目系统负责。
...
安放 sheet 对每个可枚举字段提供 4–5 个快捷答案，点击即可填入，仍可在下方编辑。承诺、下一步、关闭条件、等待对象、回收日期、唤醒条件、停止理由、出口方式和结果均遵循选择优先；项目标记可从已有值选择或自由填写；人员、产品、文章链接等无法可靠推断的信息必须输入，不能用泛化类别制造虚假去向。快捷答案只是减少输入，绝不替人自动提交或自动承诺。
...
读取必须宽容但不能假装理解：未知字段原样保留；未知 `action` 涉及的 idea 标为 `unsupported`，不参与自动转换；硬 WIP 模式只要存在未知 action，就禁止新增 `commit`。归约器只解释 `project` 的非空字符串与 `null`、以及 `done.delivery: article`，对 v1 历史中同名但未知的其他形状继续保留并忽略，不能因 1.3 开始使用这些字段就把旧台账变成只读；1.3 writer 则严格要求项目为非空字符串或 `null`，并要求文章交付有非空 `result`。固定的 Next 文档已存在但 YAML、`type`、`version` 或 `events` 不符合 v1 时进入只读修复，绝不能降级为空数组后覆盖；`version > 1` 也不能用 v1 writer 回写。
