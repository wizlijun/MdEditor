...
插件是一个纯前端 singleton 独立窗口，从「插件 → 思考 → Next」打开。当前插件协议不能注册主窗口侧栏，因此不设计常驻看板。
...
默认只展开 `WIP` 与 `Waiting`。点「安放一条」才出现最多 10 条近期 `Capture`；更旧内容仍留在 Vault，可搜索，但不显示「还有 237 条」。`Dormant`、`Closed` 和孤立记录只经搜索/筛选进入。

卡片只显示恢复行动所需的信息：标题、下一步或等待对象、必要日期，以及独立的「已有论证」徽标。不做优先级、估时、分数、标签云或拖拽。每卡只有一个「安放」按钮，打开 sheet；不做卡片 `…` popup。
