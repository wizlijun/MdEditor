...
>
> 1.5.1 扩展：WIP 警告上限改为 Next 插件设置，默认 5。完整「进行中」数量达到上限后，窗口与泳道持续显示醒目警告，直到数量降到上限以下或用户调高设置。下文「三个固定槽位」与 G3 是早期预注册实验记录，不再是当前配置契约；普通安放仍是持续警告而非统一硬拒绝。

> 1.6.0 扩展：Idea 与 Task 来源统一提供 P0–P3 优先级、真实截止日期与 GTD 情境。新卡默认 P2、无截止、情境未明确；Next 设置只预填未来新建卡片，不动态改写历史来源。截止日期不等于 Waiting 的 `review_at` 或 Dormant 的 `wake_trigger`，也不改变生命周期状态。

## 0. 直接结论

这不是一个想法看板，而是一道**承诺防火墙**：

> 把所有念头安全地留在 Vault，只把受 WIP 上限约束的当前承诺留在手上。
...
- 不做估时、所需精力、评分、通用标签云、统计仪表盘或团队协作；P0–P3 只是用户请求的静态排序线索，执行时仍应先看情境、可用时间与精力。
